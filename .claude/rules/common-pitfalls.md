...
| Change code without updating docs | Update at least one project doc per code commit |
| Skip verification script | Always run `./scripts/verify-memory-and-checks.sh` before committing |
| Hold all context in chat history | Write decisions to `docs/decisions.md`; HANDOFF.md auto-saved for session continuity |
